...
class SolanaPlatform extends sdk_connect_1.PlatformContext {
    ...
    static async getBalances(chain, rpc, walletAddress, tokens) {
        ...
        const splParsedTokenAccounts = (await Promise.all([spl_token_1.TOKEN_PROGRAM_ID, spl_token_1.TOKEN_2022_PROGRAM_ID]
            .map(pid => new web3_js_1.PublicKey(pid))
            .map(programId => rpc.getParsedTokenAccountsByOwner(new web3_js_1.PublicKey(walletAddress), { programId })))).reduce((acc, val) => {
        ...
        const balancesArr = tokens.map((token) => {
            ...
            const amount = splParsedTokenAccounts.find((v) => v?.account.data.parsed?.info?.mint === token)?.account.data.parsed?.info?.tokenAmount?.amount;
            if (!amount)
