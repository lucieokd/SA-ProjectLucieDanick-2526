...
exports.estimateFinalityTime = exports.consistencyLevelToBlock = exports.blockTime = exports.finalityThreshold = exports.safeThreshold = exports.ConsistencyLevels = void 0;
...
(function (ConsistencyLevels) {
    // SolanaConfirmed = 0,
    ConsistencyLevels[ConsistencyLevels["Finalized"] = 1] = "Finalized";
    ConsistencyLevels[ConsistencyLevels["Immediate"] = 200] = "Immediate";
    ConsistencyLevels[ConsistencyLevels["Safe"] = 201] = "Safe";
})(ConsistencyLevels || (exports.ConsistencyLevels = ConsistencyLevels = {}));
 ...
 */
function consistencyLevelToBlock(chain, consistencyLevel, fromBlock = 0n) {
    // We're done
    if (consistencyLevel === ConsistencyLevels.Immediate)
        return fromBlock;
    // Bsc is the only chain that treats consistency level as # of blocks
    if (chain === "Bsc")
        return fromBlock + BigInt(consistencyLevel);
    ...
    if (chain === "Solana" && consistencyLevel === 0)
        return fromBlock;
    // Get the number of blocks until finalized
    const chainFinality = exports.finalityThreshold.get(chain);
    if (chainFinality === undefined)
        throw new Error("Cannot find chain finality for " + chain);
    ...
    const safeRounds = exports.safeThreshold.get(chain);
    if (safeRounds === undefined)
        throw new Error("Cannot find safe threshold for " + chain);
    switch (chain) {
        case "Ethereum":
...
exports.consistencyLevelToBlock = consistencyLevelToBlock;
/**
 ...
 */
function estimateFinalityTime(chain) {
    const finality = exports.finalityThreshold.get(chain);
    if (finality === undefined)
        throw new Error("Cannot find finality for " + chain);
    const time = exports.blockTime.get(chain);
    if (time === undefined)
        throw new Error("Cannot find block time for " + chain);
    return finality * time;
}
exports.estimateFinalityTime = estimateFinalityTime;
