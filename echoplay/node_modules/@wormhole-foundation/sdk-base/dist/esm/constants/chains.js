...
export const isChain = (chain) => chainToChainId.has(chain);
export const isChainId = (chainId) => chainIdToChain.has(chainId);
...
export const asChainId = (chainId) => {
    assertChainId(chainId);
    return chainId;
};
export const toChainId = (chain) => {
    switch (typeof chain) {
        case "string":
            if (isChain(chain))
...
export const toChain = (chain) => {
    switch (typeof chain) {
        ...
        case "number":
            if (isChainId(chain))
                return chainIdToChain(chain);
            break;
        case "bigint":
            if (isChainId(Number(chain)))
                return chainIdToChain.get(Number(chain));
            break;
    }
