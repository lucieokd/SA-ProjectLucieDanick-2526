...
(function (CircleTransfer) {
    ...
    async function destinationOverrides(srcChain, dstChain, transfer) {
        ...
        if (transfer.to.chain === "Solana" && !_transfer.automatic) {
            const usdcAddress = Wormhole.parseAddress("Solana", circle.usdcContract.get(dstChain.network, dstChain.chain));
    ...
    async function quoteTransfer(srcChain, dstChain, transfer) {
        ...
        const eta = (srcChain.chain === "Polygon" ? 2_000 * 200 : finality.estimateFinalityTime(srcChain.chain)) + guardians.guardianAttestationEta;
        if (!transfer.automatic) {
            return {
