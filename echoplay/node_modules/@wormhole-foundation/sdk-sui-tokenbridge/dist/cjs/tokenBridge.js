...
class SuiTokenBridge {
    ...
    async getOriginalAsset(token) {
        ...
        if (!(0, sdk_sui_2.isValidSuiType)(coinType))
        ...
        const fields = (0, sdk_sui_2.getFieldsFromObjectResponse)(res);
        if (!fields)
            throw (0, sdk_connect_1.ErrNotWrapped)(coinType);
        if (!(0, sdk_sui_2.isMoveStructObject)(fields))
        ...
        if (!(0, sdk_sui_2.isMoveStructStruct)(val))
            throw new Error("Expected fields to be a MoveStruct");
        // Normalize types
        const type = (0, sdk_sui_2.trimSuiType)(val.type);
        coinType = (0, sdk_sui_2.trimSuiType)(coinType);
        ...
        if (type.includes(`wrapped_asset::WrappedAsset<${coinType}>`)) {
            ...
            if (!(0, sdk_sui_2.isMoveStructStruct)(info))
                throw new Error("Expected fields to be a MoveStruct");
            const address = info.fields["token_address"];
            if (!(0, sdk_sui_2.isMoveStructStruct)(address))
                throw new Error("Expected fields to be a MoveStruct");
            if (!(0, sdk_sui_2.isMoveStructObject)(address.fields))
                throw new Error("Expected address data to be a MoveObject");
            if (!("value" in address.fields))
                throw new Error("Expected a `value` key in fields of MoveStruct");
            const addressVal = address.fields["value"];
            if (!(0, sdk_sui_2.isMoveStructStruct)(addressVal))
                throw new Error("Expected fields to be a MoveStruct");
    ...
    async getTokenNativeAddress(originChain, token) {
        const address = await (0, utils_js_1.getTokenCoinType)(this.provider, this.tokenBridgeObjectId, token.toUint8Array(), (0, sdk_connect_1.toChainId)(originChain));
        if (!address)
            throw new Error(`Token ${token.toString()} not found in token registry`);
        return new sdk_sui_1.SuiAddress(address);
    }
    ...
    async isTransferCompleted(vaa) {
        const tokenBridgeStateFields = await (0, sdk_sui_2.getObjectFields)(this.provider, this.tokenBridgeObjectId);
        if (!tokenBridgeStateFields)
            throw new Error("Unable to fetch object fields from token bridge state");
        const hashes = tokenBridgeStateFields["consumed_vaas"]?.fields?.hashes;
        const keyType = (0, sdk_sui_2.getTableKeyType)(hashes?.fields?.items?.type);
        if (!keyType)
    ...
    async *submitAttestation(vaa, sender) {
        ...
        const publishTx = await (0, sdk_sui_2.publishPackage)(build, senderAddress);
        ...
        while (!found) {
            ...
            for (const txb of txBlocks.data) {
                ...
                for (const change of txb.objectChanges) {
                    if ((0, sdk_sui_2.isSuiPublishEvent)(change) && change.packageId !== undefined) {
                        coinPackageId = change.packageId;
                    }
                    else if ((0, sdk_sui_2.isSuiCreateEvent)(change) && change.objectType.includes("WrappedAssetSetup")) {
                        wrappedSetupObjectId = change.objectId;
                        // TODO: what
                        versionType = change.objectType.split(", ")[1].replace(">", ""); // ugh
                    }
                    else if ((0, sdk_sui_2.isSuiCreateEvent)(change) && change.objectType.includes("UpgradeCap")) {
                        coinUpgradeCapId = change.objectId;
                    }
                    else if ((0, sdk_sui_2.isSuiCreateEvent)(change) && change.objectType.includes("CoinMetadata")) {
        ...
        const coinType = (0, sdk_sui_2.getCoinTypeFromPackageId)(coinPackageId);
        ...
        const [txVaa] = createTx.moveCall({
            ...
            arguments: [
                ...
                createTx.pure((0, sdk_sui_2.uint8ArrayToBCS)((0, sdk_connect_1.serialize)(vaa))),
    ...
    async *transfer(sender, recipient, token, amount, payload) {
        ...
        const coins = await sdk_sui_2.SuiPlatform.getCoins(this.provider, sender, coinType);
        const [primaryCoin, ...mergeCoins] = coins.filter((coin) => (0, sdk_sui_2.isSameType)(coin.coinType, coinType));
        ...
        if (!payload) {
            const [transferTicket, dust] = tx.moveCall({
                ...
                arguments: [
                    ...
                    tx.pure((0, sdk_sui_2.uint8ArrayToBCS)(recipient.address.toUint8Array())),
                    tx.pure(relayerFee),
                    tx.pure(nonce),
                ],
                typeArguments: [coinType],
            ...
            const emitterCap = await (async () => {
                const objectId = await (0, sdk_sui_2.getOldestEmitterCapObjectId)(this.provider, coreBridgePackageId, senderAddress);
                if (objectId !== null) {
    ...
    async *redeem(sender, vaa, unwrapNative = true) {
        ...
        const [verifiedVAA] = tx.moveCall({
            ...
            arguments: [
                tx.object(this.coreBridgeObjectId),
                tx.pure((0, sdk_sui_2.uint8ArrayToBCS)((0, sdk_connect_1.serialize)(vaa))),
                tx.object(utils_1.SUI_CLOCK_OBJECT_ID),
            ],
        });
    ...
    async getPackageIds() {
        ...
        return Promise.all([
            (0, sdk_sui_2.getPackageId)(this.provider, this.coreBridgeObjectId),
            (0, sdk_sui_2.getPackageId)(this.provider, this.tokenBridgeObjectId),
        ]);
    }
    async getCoinBuildOutput(coreBridgePackageId, tokenBridgePackageId, decimals) {
        ...
        const strippedTokenBridgePackageId = (await (0, sdk_sui_2.getOriginalPackageId)(this.provider, this.tokenBridgeObjectId))?.replace("0x", "");
    ...
    createUnsignedTx(txReq, description, parallelizable = false) {
        return new sdk_sui_2.SuiUnsignedTransaction(txReq, this.network, this.chain, description, parallelizable);
    }
}
