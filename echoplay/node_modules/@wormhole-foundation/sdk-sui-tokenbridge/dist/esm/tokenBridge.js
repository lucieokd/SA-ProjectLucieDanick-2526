...
import { ErrNotWrapped, UniversalAddress, canonicalAddress, encoding, isNative, keccak256, nativeChainIds, serialize, toChain, toChainId, toNative, } from "@wormhole-foundation/sdk-connect";
import { SuiAddress } from "@wormhole-foundation/sdk-sui";
import { SuiPlatform, SuiUnsignedTransaction, getCoinTypeFromPackageId, getFieldsFromObjectResponse, getObjectFields, getOldestEmitterCapObjectId, getOriginalPackageId, getPackageId, getTableKeyType, isMoveStructObject, isMoveStructStruct, isSameType, isSuiCreateEvent, isSuiPublishEvent, isValidSuiType, publishPackage, trimSuiType, uint8ArrayToBCS, } from "@wormhole-foundation/sdk-sui";
import { getTokenCoinType, getTokenFromTokenRegistry } from "./utils.js";
import "@wormhole-foundation/sdk-sui-core";
export class SuiTokenBridge {
    network;
    chain;
    provider;
    contracts;
    ...
    async getOriginalAsset(token) {
        ...
        if (type.includes(`wrapped_asset::WrappedAsset<${coinType}>`)) {
            ...
            if (!isMoveStructStruct(info))
                throw new Error("Expected fields to be a MoveStruct");
            const address = info.fields["token_address"];
            if (!isMoveStructStruct(address))
                throw new Error("Expected fields to be a MoveStruct");
            ...
            if (!isMoveStructStruct(addressVal))
                throw new Error("Expected fields to be a MoveStruct");
    ...
    async getTokenNativeAddress(originChain, token) {
        const address = await getTokenCoinType(this.provider, this.tokenBridgeObjectId, token.toUint8Array(), toChainId(originChain));
        if (!address)
    ...
    async getWrappedAsset(token) {
        ...
        const address = await getTokenCoinType(this.provider, this.tokenBridgeObjectId, token.address.toUniversalAddress().toUint8Array(), toChainId(token.chain));
        if (!address)
            throw ErrNotWrapped(canonicalAddress(token));
    ...
    async *transfer(sender, recipient, token, amount, payload) {
        ...
        if (!payload) {
            ...
            const emitterCap = await (async () => {
                const objectId = await getOldestEmitterCapObjectId(this.provider, coreBridgePackageId, senderAddress);
                if (objectId !== null) {
    ...
    async *redeem(sender, vaa, unwrapNative = true) {
        const coinType = await getTokenCoinType(this.provider, this.tokenBridgeObjectId, vaa.payload.token.address.toUint8Array(), toChainId(vaa.payload.token.chain));
        if (!coinType) {
    ...
    async getCoinBuildOutput(coreBridgePackageId, tokenBridgePackageId, decimals) {
        ...
        const strippedTokenBridgePackageId = (await getOriginalPackageId(this.provider, this.tokenBridgeObjectId))?.replace("0x", "");
        ...
        return {
            modules: [bytecode],
            dependencies: ["0x1", "0x2", tokenBridgePackageId, coreBridgePackageId].map((d) => normalizeSuiObjectId(d)),
        };
    }
