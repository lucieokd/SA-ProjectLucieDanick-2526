...
describe('RewardManagerProgram', () => {
    it('encodes the Discovery attestation instruction data correctly', () => {
        ...
        expect(data.subarray(0, 20).toString('hex')).toBe(args.recipientEthAddress.substring(2).toLowerCase());
        ...
        expect(data.subarray(45, 65).toString('hex')).toBe(args.antiAbuseOracleEthAddress.substring(2).toLowerCase());
    });
    it('decodes the Discovery attestation instruction data correctly', () => {
        ...
        expect(attestation.recipientEthAddress).toBe(args.recipientEthAddress.toLowerCase());
        expect(attestation.amount).toBe(args.amount);
        expect(attestation.disbursementId).toBe(args.disbursementId);
        expect(attestation.antiAbuseOracleEthAddress).toBe(args.antiAbuseOracleEthAddress.toLowerCase());
    });
    ...
    it('decodes the account data of a single attestation', () => {
        ...
        expect(decoded.messages[0].attestation.recipientEthAddress).toBe('0x34017d7abdf9e0cbc337980ae1763e0a49777df7');
        expect(decoded.messages[0].attestation.amount).toBe(BigInt(100000000));
        expect(decoded.messages[0].attestation.disbursementId).toBe('track-upload:483117431');
    ...
    it('encodes the evaluate attestation instruction data', () => {
        const mockPubkey = new PublicKey('7c7wdSMAvswavryV6d9knEskoptUx919F2bLFYPrffqQ');
        const instruction = RewardManagerProgram.createEvaluateAttestationsInstruction({
        ...
        expect(instruction.data.subarray(1, 9).readBigInt64LE()).toBe(BigInt('100000000'));
        expect(instruction.data.subarray(9).readUint16LE()).toBe(instruction.data.byteLength - 20 - 9 - 4 // ethAddress, curOffset, size of this number
        );
        expect(instruction.data
            .subarray(13, instruction.data.byteLength - 20)
            .toString('utf-8')).toBe('track-upload:483117431');
        expect(instruction.data
            .subarray(instruction.data.byteLength - 20)
            .toString('hex')).toBe('34017d7abdf9e0cbc337980ae1763e0a49777df7');
        expect(instruction.data.byteLength).toBe(55);
    });
});
//# sourceMappingURL=RewardManagerProgram.test.js.map