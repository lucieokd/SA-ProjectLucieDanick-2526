 ...
 */
const createTransactionWithSigners = ({ signers, chainId, message, timeoutHeight, memo = '', fee = (0, utils_2.getDefaultStdFee)(), signMode = index_js_3.SIGN_DIRECT, }) => {
    const actualSigners = Array.isArray(signers) ? signers : [signers];
    const [signer] = actualSigners;
    const body = (0, index_js_3.createBody)({ message, memo, timeoutHeight });
    const actualFee = typeof fee === 'string' ? (0, utils_3.getStdFeeFromString)(fee) : fee;
    const feeMessage = (0, index_js_3.createFee)({
        fee: actualFee.amount[0],
        payer: actualFee.payer,
        granter: actualFee.granter,
        gasLimit: parseInt(actualFee.gas, 10),
    });
    const signInfo = (0, index_js_3.createSigners)({
        chainId,
        mode: signMode,
        signers: actualSigners,
    });
    const authInfo = (0, index_js_3.createAuthInfo)({
        signerInfo: signInfo,
        fee: feeMessage,
    });
    ...
    const signDoc = (0, index_js_3.createSignDoc)({
        chainId,
        bodyBytes: bodyBytes,
    ...
    const toSignHash = (0, viem_1.keccak256)(Buffer.from(signDocBytes), 'bytes');
...
const createTransactionForAddressAndMsg = async (params) => {
    const messages = Array.isArray(params.message)
        ...
        : [params.message];
    // Clients
    const chainRestAuthApi = new index_js_2.ChainRestAuthApi(params.endpoint);
    const tendermintRestApi = new index_js_2.ChainRestTendermintApi(params.endpoint);
    /** Account Details * */
    const accountDetails = await chainRestAuthApi.fetchCosmosAccount(params.address);
    const baseAccount = index_js_1.BaseAccount.fromRestCosmosApi(accountDetails);
    /** Block Details */
