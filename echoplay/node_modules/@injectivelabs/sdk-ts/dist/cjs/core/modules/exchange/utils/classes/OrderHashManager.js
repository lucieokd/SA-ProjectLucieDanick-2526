...
class OrderHashManager {
    ...
    hashTypedData(eip712) {
        const bytesToHash = Buffer.concat([
            Buffer.from('19', 'hex'),
            Buffer.from('01', 'hex'),
            Buffer.from((0, crypto_js_1.domainHash)(eip712)),
            Buffer.from((0, crypto_js_1.messageHash)(eip712)),
        ]);
        try {
            return `0x${Buffer.from((0, viem_1.keccak256)(bytesToHash)).toString('hex')}`;
        }
        catch {
            return '';
        }
