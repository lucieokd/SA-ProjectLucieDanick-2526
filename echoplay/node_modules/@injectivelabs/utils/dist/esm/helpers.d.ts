 ...
 */
export declare const awaitAll: <T, S>(array: Array<T>, callback: (item: T) => Promise<S>) => Promise<Awaited<S>[]>;
/**
 * When we want to execute the promises one by one
 * and not all in batch as with await Promise.all()
 */
export declare const awaitForAll: <T, S>(array: Array<T>, callback: (item: T) => Promise<S>) => Promise<S[]>;
export declare const splitArrayToChunksThrow: <T>({ array, filter, chunkSize, }: {
    array: Array<T>;
    chunkSize: number;
    filter?: (item: T) => boolean;
}) => T[][];
export declare const splitArrayToChunks: <T>({ array, filter, chunkSize, }: {
    array: Array<T>;
    chunkSize: number;
    filter?: (item: T) => boolean;
}) => T[][];
